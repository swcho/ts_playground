...
import { Buffer, Part, Transport } from 'tone';
...
Buffer.on('progress', (prog) => {
    (document.querySelector('#loading #fill') as HTMLElement).style.width = (prog * 100).toString() + '%';
});
 ...
 *  MIDI FILE
 */
load('clair_de_lune.mid').then((midi) => {
...
window.addEventListener('DOMContentLoaded', () => {
    let inputs = [].slice.call(this, document.querySelectorAll('input'));
