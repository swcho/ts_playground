...
// https://codepen.io/liabru/pen/jwXqOd
...
Example.ragdoll = function() {
    ...
    // create obstacles
    let obstacles = Composites.stack(300, 0, 15, 3, 10, 10, function(x, y, column) {
    ...
    for (let i = 0; i < 1; i += 1) {
        let ragdoll = Example.ragdoll.ragdoll(200, -1000 * i, 1.3);
...
Example.ragdoll.ragdoll = function(x, y, scale, options) {
    ...
    let head = Bodies.rectangle(x, y - 60 * scale, 34 * scale, 40 * scale, headOptions);
    let chest = Bodies.rectangle(x, y, 55 * scale, 80 * scale, chestOptions);
    let rightUpperArm = Bodies.rectangle(x + 39 * scale, y - 15 * scale, 20 * scale, 40 * scale, rightArmOptions);
    let rightLowerArm = Bodies.rectangle(x + 39 * scale, y + 25 * scale, 20 * scale, 60 * scale, rightLowerArmOptions);
    let leftUpperArm = Bodies.rectangle(x - 39 * scale, y - 15 * scale, 20 * scale, 40 * scale, leftArmOptions);
    let leftLowerArm = Bodies.rectangle(x - 39 * scale, y + 25 * scale, 20 * scale, 60 * scale, leftLowerArmOptions);
    let leftUpperLeg = Bodies.rectangle(x - 20 * scale, y + 57 * scale, 20 * scale, 40 * scale, leftLegOptions);
    let leftLowerLeg = Bodies.rectangle(x - 20 * scale, y + 97 * scale, 20 * scale, 60 * scale, leftLowerLegOptions);
    let rightUpperLeg = Bodies.rectangle(x + 20 * scale, y + 57 * scale, 20 * scale, 40 * scale, rightLegOptions);
...
//       sourceLink: 'https://github.com/liabru/matter-js/blob/master/examples/ragdoll.js'
//     }
//   ]
// });
