...
void main(){
    ...
    float shapeRatio = uMove ? cos(time * 1.5 + sin(time * 1.5)) * 0.5 + 0.5 : uShapeRatio;

    vec3 shape1 = uSphere ? paramFunction1() * 1.5 : vec3(1, 1, 1);
    vec3 shape2 = uMobius ? paramFunction2() : vec3(1, 1, 1);
    ...
    vec4 mvPos = uRotation ? vec4(rotatePos, 1.0) : vec4(scalePos, 1.0);

    vPos = mvPos.xyz;
    // vNormal = normal;

    // trials
    // mvPos = vec4(shape1, 1.0);
