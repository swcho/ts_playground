...
// https://codepen.io/interaminense/pen/pwmNWa

import './style.scss';
import * as React from 'react';
import * as ReactDOM from 'react-dom';

class Thermometer extends React.Component<any, any> {
    constructor(props) {
        super(props);
        ...
        for (let step = 0; step <= this.state.steps; step++) {
            let val = parseFloat(((this.state.max / this.state.steps) * step).toFixed(2));
...
ReactDOM.render(
    <div className='example'>
        <Thermometer theme={'dark'} value={180} max={200} format={'$'} steps={3} size={'small'} height={180} />
        <Thermometer theme={'dark'} value={180} max={200} format={'$'} steps={3} size={'normal'} height={180} />
        <Thermometer theme={'dark'} value={180} max={200} format={'$'} steps={3} size={'large'} height={180} />
    </div>,
