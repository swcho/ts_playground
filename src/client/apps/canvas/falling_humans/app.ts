...
// https://codepen.io/ge1doot/pen/EvYpye
...
class Human {
    points: Human.Point[];
    constraints: Human.ConstraintBase[];
    shapes: Human.Shape[];
...
namespace Human {
    ...
    export class Point {
        ...
        constructor(x: number, y: number, p: PointInfo, size: number, gravity: number) {
            this.x = x + p.x * size;
            this.y = y + p.y * size;
            this.px = this.x;
            this.py = this.y;
            this.vx = 0.0;
            this.vy = 0.0;
            this.mass = p.mass || 1.0;
            this.gravity = gravity;
        }
        join(p1: PointInfo, distance: number, force: number) {
            const dx = p1.x - this.x;
            const dy = p1.y - this.y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            const tw = this.mass + p1.mass;
            const r1 = p1.mass / tw;
            const r0 = this.mass / tw;
        ...
        integrate() {
            ...
            this.px = this.x;
            this.py = this.y;
            this.x += this.vx + this.gravity * dir;
    ...
    export class Shape {
        ...
        private height;
        private offset;
        private shape: HTMLCanvasElement;
        constructor(p0: Point, p1: Point, shape: ShapeInfo, svg, size) {
            this.p0 = p0;
            this.p1 = p1;
    ...
    export class Constraint implements ConstraintBase {
        private distance;
        private force;
        constructor(private p0: Point, private p1: Point, constraint) {
    ...
    export class Angle implements ConstraintBase {
        private p0;
        private p1;
        private p2;
        private len1;
