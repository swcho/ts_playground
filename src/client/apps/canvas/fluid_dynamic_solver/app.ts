...
class FluidDynamicSolver {
    ...
    advection(prev, next, u_next, v_next, dt, boundaryFlag) {
        ...
        for (let i = 1; i <= this.width; i++) {
            for (let j = 1; j <= this.height; j++) {
                ...
                next[this.idx(i, j)] =
                    s0 *
                        (t0 * prev[this.idx(i0, j0)] +
                            t1 * prev[this.idx(i0, j1)]) +
                    s1 *
                        (t0 * prev[this.idx(i1, j0)] +
                            t1 * prev[this.idx(i1, j1)]);
            }
        }
    ...
    setBoundary(target, boundaryFlag) {
        ...
        target[this.idx(0, this.height + 1)] =
            (target[this.idx(0, this.height)] +
                target[this.idx(1, this.height + 1)]) /
            2;
        target[this.idx(this.width + 1, 0)] =
            (target[this.idx(this.width, 0)] +
                target[this.idx(this.width + 1, 1)]) /
            2;
...
function pointerMoveHandle(x, y) {
    ...
    if (lastX !== undefined) {
        ...
        if (control.emitDensity) {
            for (
                let i = Math.max(1, tileX - control.emitDensityExpand);
                i <= Math.min(tileX + control.emitDensityExpand, col);
                i++
            ) {
                for (
                    let j = Math.max(1, tileY - control.emitDensityExpand);
                    j <= Math.min(tileY + control.emitDensityExpand, row);
                    j++
                ) {
                    d_ui[fluid.idx(i, j)] +=
                        Math.sqrt(deltaX * deltaX + deltaY * deltaY) *
                        control.emitDensityScale;
                }
            }
        }
        ...
        if (control.emitVelocity) {
            for (
                let i = Math.max(1, tileX - control.emitVelocityExpand);
                i <= Math.min(tileX + control.emitVelocityExpand, col);
                i++
            ) {
                for (
                    let j = Math.max(1, tileY - control.emitVelocityExpand);
                    j <= Math.min(tileY + control.emitVelocityExpand, row);
                    j++
                ) {
...
const Stats = require('stats.js') as any;
